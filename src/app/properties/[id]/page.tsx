...
export default async function PropertyPage({
  ...
  return (
    <>
      ...
      {/*
        Six concerns, one at a time. Stacked they were six screens of
        scrolling to reach the one you came for, with no way to tell which
        held anything until you got there — the counts say so now.
      */}
      <SectionTabs
        sections={[
          { key: 'ownership', label: 'Ownership', count: share?.paths.length ?? 0, content: (
              <RecordSection
              bare
              title="Ownership"
          ...
          { key: 'management', label: 'Management', count: property.managementPeriods.length, content: (
              <RecordSection
              bare
          ...
          { key: 'loans', label: 'Loans', count: property.loans.length, content: (
              <RecordSection
              bare
              title="Loans"
          ...
          ...(property.unitStructure === 'units'
            ? [{ key: 'leases', label: 'Leases', count: property.leases.length, content: (
              <RecordSection
              bare
              title="Leases"
            ...
            : []),
          { key: 'valuations', label: 'Valuations', count: property.valuations.length, content: (
              <RecordSection
              bare
              title="Valuations"
          ...
          { key: 'accounts', label: 'Bank accounts', count: property.bankAccounts.length, content: (
              <RecordSection
              bare
              title="Bank accounts"
      ...
      <Note tone="muted">
        Occupancy, collections and what each room earns for this house are on{' '}
        <Link href={`/operations?property=${property.id}`} className="underline">Operations</Link>, where the period
        can be changed. This page is what the house <em>is</em>; that one is how it is doing.
      </Note>
    </>
  );
}
